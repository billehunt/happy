...
export const ja: TranslationStructure = {
    ...
    time: {
        ...
        compactMinutes: ({ count }: { count: number }) => `${count}分`,
        compactHours: ({ count }: { count: number }) => `${count}時間`,
        compactDays: ({ count }: { count: number }) => `${count}日`,
    },
    ...
    session: {
        ...
        renameMessage: '空にすると自動的な名前に戻ります。',
        ...
        deleteConfirmMessage: 'この操作はセッションとそのメッセージを完全に削除します。元に戻すことはできません。',
    ...
    sidebar: {
        ...
        searchNoResults: ({ query }: { query: string }) => `"${query}" に一致するセッションはありません`,
        limitToProjects: 'プロジェクトで絞り込む',
        allProjects: 'すべてのプロジェクト',
        showOnlyProject: 'このプロジェクトのみ表示',
        newSessionHere: 'ここに新しいセッション',
        sortModeProject: 'プロジェクトごとにグループ化',
