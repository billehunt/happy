...
export const ru: TranslationStructure = {
    ...
    time: {
        ...
        daysAgo: ({ count }: { count: number }) => `${count} ${plural({ count, one: 'день', few: 'дня', many: 'дней' })} назад`,
        compactMinutes: ({ count }: { count: number }) => `${count} мин`,
        compactHours: ({ count }: { count: number }) => `${count} ч`,
        compactDays: ({ count }: { count: number }) => `${count} д`,
    },
    ...
    session: {
        ...
        renameMessage: 'Оставьте пустым, чтобы вернуться к автоматическому названию.',
        ...
        deleteConfirmMessage: 'Это навсегда удалит сессию и её сообщения. Это действие нельзя отменить.',
    ...
    sidebar: {
        ...
        searchNoResults: ({ query }: { query: string }) => `Нет сессий, соответствующих "${query}"`,
        limitToProjects: 'Ограничить проектами',
        allProjects: 'Все проекты',
        showOnlyProject: 'Показать только этот проект',
        newSessionHere: 'Новая сессия здесь',
        sortModeProject: 'Сгруппировано по проекту',
