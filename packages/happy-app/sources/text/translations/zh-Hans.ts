...
export const zhHans: TranslationStructure = {
    ...
    time: {
        ...
        compactMinutes: ({ count }: { count: number }) => `${count}分钟`,
        compactHours: ({ count }: { count: number }) => `${count}小时`,
    ...
    session: {
        ...
        deleteConfirmMessage: '此操作将永久删除会话及其消息，且无法撤销。',
        deleteFailed: '删除会话失败',
    },
    ...
    sidebar: {
        ...
        searchNoResults: ({ query }: { query: string }) => `没有与 "${query}" 匹配的会话`,
        limitToProjects: '限制为特定项目',
        allProjects: '所有项目',
        showOnlyProject: '仅显示此项目',
        newSessionHere: '在此新建会话',
        sortModeProject: '按项目分组',
